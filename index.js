...
function preload() {
  ...
  // Dialog plugin
  // this.load.plugin('DialogPlugin', './dialog_plugin.js', true);
}
...
function create() {
  ...
  zone1 = this.add.zone(spawnPoint.x, spawnPoint.y, 64, 64);
  this.physics.world.enable(zone1, 0);  // (0) DYNAMIC (1) STATIC
  this.physics.add.overlap(player, [zone1]);
  
  // Dialog plugin
  let dialogPlugin = this.plugins.install('dialogPlugin', DialogPlugin, true);
  
  zone1.on('enterzone', () => dialogPlugin.showDialog('Entering zone', this));
  zone1.on('leavezone', () => dialogPlugin.hideDialog(this));
...
function update(time, delta) {
  ...
  var touching = zone1.body.touching;
  var isEmbedded = zone1.body.embedded;
  ...
  if (!wasEmbedded) {
    ...
    if (!isEmbedded && touching.none) {
      zone1.emit('leavezone');
      wasEmbedded = false;
    }
  }
  
  zone1.body.debugBodyColor = isEmbedded ? 0x00ffff : 0xffff00;
}
